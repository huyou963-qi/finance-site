...
- `GET <origin>/robots.txt`：记录目标路径是否 Disallow。对于公开、无需认证的政府统计发布页，`robots.txt` 的 Disallow 是风险信号而非自动终止条件，但仅可在任务所有者明确授权后继续；Spec §3.1 必须记录授权、数据公共性、抓取范围与频率控制；
- 页面条款是否禁止自动抓取；抓取频率是否 ≤ 数据发布频率（月频指标每天最多 probe 一次，发布日按日历精确触发）；
- 需要 Cookie/UA 的（如 TE）用 `.env.local` 变量（`TE_CALENDAR_COOKIE` 模式），**不写进代码**。
- 公开官方统计页在获授权后，历史回填应采用限速、缓存、可恢复的方式；后续更新只在发布周期内低频 probe，禁止并发轰炸或规避访问控制；
- 不可抓（页面条款明确禁止自动抓取、需登录付费、或需要绕过技术访问控制）→ 停下，把指标改报 `manual` 或换源，回 Agent A。
