...
async function main() {
  ...
  const targets = await prisma.equitySecurity.findMany({where:{
    cik:{not:null},
    ...(symbols.length?{symbol:{in:symbols}}:{}),
    ...(trackedOnly?{metadata:{path:["ownershipCoverage"],not:Prisma.DbNull}}:{}),
  },orderBy:[{marketCap:"desc"},{symbol:"asc"}],...(limit>0?{take:Math.floor(limit)}:{}),select:{symbol:true}});
