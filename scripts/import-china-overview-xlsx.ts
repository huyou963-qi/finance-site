...
import {
  CHINA_OVERVIEW_CATEGORY_CODE_BY_NAME,
  CHINA_OVERVIEW_CATEGORY_SORT_BY_NAME,
  CHINA_OVERVIEW_SERIES,
  normalizeChinaOverviewName,
} from "../src/lib/data/chinaOverviewLayout";

loadEnvConfig(process.cwd());

const prisma = new PrismaClient();

const DEFAULT_XLSX_PATH = "C:/Users/Administrator/Desktop/模板/China_Overview.xlsx";
const SOURCE_TAG = "china-overview-xlsx";
const COUNTRY_CODE = "CN";
const COUNTRY_NAME_ZH = "中国";
...
function parseWorkbookSeries(sheetName: string, rows: unknown[][]): SeriesSeed[] {
  ...
  for (const def of CHINA_OVERVIEW_SERIES) {
    ...
    if (normalizeChinaOverviewName(headerName) !== normalizeChinaOverviewName(def.displayName)) {
      console.warn(
...
async function main() {
  ...
  const cnCategoryId = await upsertCategory("macro_country_cn", "中国", rootCategoryId, 20);

  const themeCategoryIds = new Map<string, string>();
  for (const name of new Set(CHINA_OVERVIEW_SERIES.map((row) => row.catalogCategory))) {
    const slug = CHINA_OVERVIEW_CATEGORY_CODE_BY_NAME[name];
    if (!slug) throw new Error(`missing category code mapping: ${name}`);
    const id = await upsertCategory(
      `macro_country_cn_${slug}`,
      name,
      cnCategoryId,
      CHINA_OVERVIEW_CATEGORY_SORT_BY_NAME[name] ?? 500,
    );
  ...
  if (seriesList.length !== CHINA_OVERVIEW_SERIES.length) {
    console.warn(
      `[warn] expected ${CHINA_OVERVIEW_SERIES.length} series, parsed ${seriesList.length}`,
    );
  }

  for (const series of seriesList) {
    const panelCategoryId = themeCategoryIds.get(series.categoryName);
    if (!panelCategoryId) throw new Error(`missing theme category ${series.categoryName}`);
    ...
    const instrument = await prisma.instrument.upsert({
      ...
      create: {
        code: series.code,
        kind: InstrumentKind.MACRO_SERIES,
        name: `China Overview:${series.displayName}`,
        shortName: series.displayName,
        description: "China Overview 导入序列",
