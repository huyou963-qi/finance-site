...
async function main() {
  ...
  const mergedAttrs = attrsList.filter((row) => CHINA_OVERVIEW_BY_CODE.has(row.code));
  ...
  console.info(
    `[done] workbook=${workbook} parsed=${parsedRaw} unique=${mergedAttrs.length} updated=${updated} missing=${missing}`,
  );
}
