...
async function main() { console.log(`[data:sync-safe-external] 正在获取外管局公开时间序列表 dataset=${requestedDatasets?.join(",") ?? "all"}…`); const { history, unavailable } = await fetchSafeExternalHistoryTolerant(requestedDatasets ? { datasets: requestedDatasets } : undefined); for (const item of unavailable) console.warn(`[data:sync-safe-external] ⚠ 跳过 dataset=${item.dataset}（官方来源不可用或结构已变化）：${item.message}`); let upserted = 0; for (const series of history.values()) { const item = await prisma.instrument.findUnique({ where: { code: series.code }, select: { id: true } }); if (!item) throw new Error(`未找到 ${series.code}，请先运行 data:seed-safe-external`); upserted += (await upsertMacroObservations(prisma, item.id, series.points)).upserted; } console.log(`[data:sync-safe-external] 完成：序列=${history.size}，upserted=${upserted}`); }
