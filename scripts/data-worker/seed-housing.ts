...
import {
  HOUSING_FRED_REUSED,
  HOUSING_FRED_SERIES,
  buildHousingInstrumentMetadata,
  releaseRuleForHousingFred,
...
async function main() {
  console.log("[data:seed-housing] 确保 FRED 数据源…");
  ...
  console.log("[data:seed-housing] 写入住房与地产 FRED 序列与订阅…");
  for (const row of HOUSING_FRED_SERIES) {
    const rule = releaseRuleForHousingFred(row);
    ...
    const existingMeta =
      existing?.metadata && typeof existing.metadata === "object" && !Array.isArray(existing.metadata)
        ...
        : null;
    const metadata = buildHousingInstrumentMetadata(row, {
      dataLastObsDateIso,
  ...
  console.log("[data:seed-housing] 复用序列存在性检查（不覆盖既有 seed，仅补空 unit）…");
  let reusedMissing = 0;
  ...
  console.log(
    `[data:seed-housing] 完成 created=${created} updated=${updated} reused-missing=${reusedMissing}`,
  );
  console.log(
    "  下一步：npm run data:seed-release-packages && npm run data:sync-catalog-layout -- --prefix=fred: && npm run data:backfill-empty",
  );
