...
import { SAFE_EXTERNAL_SOURCE, SAFE_EXTERNAL_SYNC_SCRIPT } from "../../src/lib/data/scheduler/safeExternal/catalog";
...
async function main() {
  ...
  const history = await fetchSafeExternalHistory(); const probedAt = new Date().toISOString();
  ...
  const stale = await prisma.instrument.findMany({
    where: { code: { startsWith: "safe_cn_", notIn: activeCodes } },
    select: { id: true },
  });
  ...
  console.log(`[data:seed-safe-external] 完成：${history.size} 条外管局官方序列，清理过期解析序列=${stale.length}`);
}
