/**
 * NY Fed 全球供应链压力指数（GSCPI）抓取——自检
 *
 * npm run data:verify-nyfed-gscpi
 * npm run data:verify-nyfed-gscpi -- --db
 */
...
async function main() {
  ...
  const code = NYFED_GSCPI_INSTRUMENT.code;
  console.log(`[verify-nyfed-gscpi] 目标仪器 ${code}`);

  if (!useDb) {
    console.log("[verify-nyfed-gscpi] 通过（加 --db 检查数据库）");
    return;
  }
  ...
  try {
    ...
    if (!inst) {
      console.error(`  ✗ 缺 Instrument ${code}（先 data:seed-nyfed-gscpi）`);
      process.exit(1);
    }
    ...
    if (scrape?.provider !== "nyfed_gscpi") {
      console.error(`  ✗ scrape.provider=${scrape?.provider ?? "无"}（应 nyfed_gscpi）`);
      errors++;
    } else {
      console.log(`  ✓ scrape.provider=nyfed_gscpi · url=${scrape.url}`);
    }
    ...
    if (count < 300) {
      console.error(`  ✗ 观测仅 ${count} 条（预期 ≥300，全历史约 343，1998-01 起）`);
      errors++;
    } else {
      console.log(
    ...
    // 值域检查：GSCPI 标准化指数合理区间 [-10,10]
    const bad = await prisma.macroObservation.count({
      where: { instrumentId: inst.id, OR: [{ value: { lt: -10 } }, { value: { gt: 10 } }] },
    });
    if (bad > 0) {
      console.error(`  ✗ ${bad} 条观测值超出 [-10,10]`);
      errors++;
    } else {
      console.log("  ✓ 全部观测值在 [-10,10]");
    }
  } catch (e) {
  ...
  if (errors > 0) {
    console.error(`[verify-nyfed-gscpi] 失败：${errors} 项`);
    process.exit(1);
  }
  console.log("[verify-nyfed-gscpi] 通过");
}
