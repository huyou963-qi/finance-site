...
import {
  STANDING_REPO_FRED_SERIES,
  buildStandingRepoInstrumentMetadata,
  releaseRuleForStandingRepo,
} from "../../src/lib/data/scheduler/standingRepoFredSeedCatalog";
  ...
  for (const item of STANDING_REPO_FRED_SERIES) {
    ...
    const existingMetadata =
      ...
      !Array.isArray(existing.metadata)
        ...
        : null;
    const metadata = buildStandingRepoInstrumentMetadata(item, {
    ...
    const instrument = await prisma.instrument.upsert({
      ...
      create: {
        ...
        externalRefs: {
          ...
          agencyId: "us-fred",
          sourceId: "fred",
          standingRepoCategory: item.category,
        },
      },
      update: {
        name: item.name,
    ...
    const releaseRule = releaseRuleForStandingRepo();
