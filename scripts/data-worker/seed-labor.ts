...
import {
  LABOR_FRED_IDS_ALREADY_SEEDED,
  LABOR_FRED_SERIES,
  buildLaborInstrumentMetadata,
  releaseRuleForLaborFred,
...
async function main() {
  console.log("[data:seed-labor] 确保 FRED 数据源…");
  ...
  console.log("[data:seed-labor] 写入就业 FRED 序列与订阅…");
  for (const row of LABOR_FRED_SERIES) {
    const rule = releaseRuleForLaborFred(row.fredId, row.granularity);
    const nextRunAt = computeNextRunAt(rule, new Date());
    const wasSeeded = LABOR_FRED_IDS_ALREADY_SEEDED.has(row.fredId);
    ...
    const existingMeta =
      existing?.metadata && typeof existing.metadata === "object" && !Array.isArray(existing.metadata)
        ...
        : null;
    const metadata = buildLaborInstrumentMetadata(row, {
      dataLastObsDateIso,
  ...
  console.log(
    `[data:seed-labor] 完成 created=${created} updated=${updated} skipped-existing=${skippedExisting}`,
  );
