/**
 * BEA NIPA 企业利润（税前/税后）FRED 种子。
 *
 * npm run data:seed-corporate-profits
 */
...
import {
  CORPORATE_PROFITS_FRED_SERIES,
  buildCorporateProfitsInstrumentMetadata,
  releaseRuleForCorporateProfits,
} from "../../src/lib/data/scheduler/corporateProfitsFredSeedCatalog";
  ...
  for (const item of CORPORATE_PROFITS_FRED_SERIES) {
    ...
    const existingMetadata =
      ...
      !Array.isArray(existing.metadata)
        ...
        : null;
    const metadata = buildCorporateProfitsInstrumentMetadata(item, {
    ...
    const instrument = await prisma.instrument.upsert({
      ...
      create: {
        ...
        externalRefs: {
          ...
          agencyId: "us-fred",
          sourceId: "fred",
          corporateProfitsCategory: item.category,
        },
      },
      update: {
        name: item.name,
    ...
    const releaseRule = releaseRuleForCorporateProfits();
