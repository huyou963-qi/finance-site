/**
 * 圣路易斯联储金融压力指数（STLFSI4）FRED 种子。
 *
 * npm run data:seed-financial-stress
 */
...
import {
  FINANCIAL_STRESS_FRED_SERIES,
  buildFinancialStressInstrumentMetadata,
  releaseRuleForFinancialStress,
} from "../../src/lib/data/scheduler/financialStressFredSeedCatalog";
  ...
  for (const item of FINANCIAL_STRESS_FRED_SERIES) {
    ...
    const existingMetadata =
      ...
      !Array.isArray(existing.metadata)
        ...
        : null;
    const metadata = buildFinancialStressInstrumentMetadata(item, {
    ...
    const instrument = await prisma.instrument.upsert({
      ...
      create: {
        ...
        externalRefs: {
          ...
          agencyId: "us-fred",
          sourceId: "fred",
          financialStressCategory: item.category,
        },
      },
      update: {
        name: item.name,
    ...
    const releaseRule = releaseRuleForFinancialStress();
