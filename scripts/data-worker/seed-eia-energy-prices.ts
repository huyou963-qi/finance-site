/**
 * EIA 能源价格（周度零售汽油、亨利港天然气、航油、取暖油）FRED 种子。
 *
...
import {
  ...
  buildEiaEnergyPricesInstrumentMetadata,
  ...
  for (const item of EIA_ENERGY_PRICES_FRED_SERIES) {
    ...
    const existingMetadata =
      ...
      !Array.isArray(existing.metadata)
        ...
        : null;
    const metadata = buildEiaEnergyPricesInstrumentMetadata(item, {
    ...
    const instrument = await prisma.instrument.upsert({
      ...
      create: {
        ...
        externalRefs: {
          ...
          agencyId: "us-fred",
          sourceId: "fred",
          eiaEnergyPricesCategory: item.category,
        },
      },
      update: {
        name: item.name,
    ...
    const releaseRule = releaseRuleForEiaEnergyPrices(item.granularity);
