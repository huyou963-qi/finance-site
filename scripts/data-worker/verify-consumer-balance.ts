 ...
 * 美国消费与居民资产负债自检
 *
 * npm run data:verify-consumer-balance
 * npm run data:verify-consumer-balance -- --db
 * Spec: docs/specs/us-consumer-balance.spec.md §6
 */
...
const HISTORY_DEPTH_MAX_FIRST_YEAR: Record<string, number> = {
  ...
  PCESC96: 2008,
  PSAVERT: 1960,
  // FRED API 对 Z.1 / G.19 早期点常从 1950 起返回（系列元数据更早，以 API 实测为准）
  TNWBSHNO: 1951,
  TDSP: 2006,
  TOTALSL: 1951,
  REVOLSL: 1969,
...
async function main() {
  ...
  console.log(
    `[verify-consumer-balance] 目录 ${CONSUMER_BALANCE_FRED_SERIES.length} 条新 seed + ${CONSUMER_BALANCE_FRED_REUSED.length} 条复用`,
  );

  if (!useDb) {
    console.log("[verify-consumer-balance] 通过（加 --db 检查数据库与近期观测）");
  ...
  try {
    console.log(
      "[verify-consumer-balance] 订阅启用与 releaseRule（日历型=economic_calendar，probe 型=probe_interval）",
    );
    let subsOk = 0;
    ...
    console.log(`  ✓ ${subsOk}/${CONSUMER_BALANCE_FRED_SERIES.length} 条订阅启用且 releaseRule 正确`);

    console.log("[verify-consumer-balance] 复用序列存在（phase2 seed）+ unit 完整性");
    for (const reused of CONSUMER_BALANCE_FRED_REUSED) {
      ...
      if (!inst.unit?.trim()) {
        console.error(`  ✗ ${reused.code} unit 为空（运行 data:seed-consumer-balance 回填）`);
        errors++;
        continue;
      }
    ...
    console.log("[verify-consumer-balance] 发布包归属（按 FRED 官方 Release 分组）");
    let pkgOk = 0;
    const pkgExpectations = [
      ...CONSUMER_BALANCE_FRED_SERIES.map((r) => ({ code: r.code, expected: r.releasePackageId })),
      ...CONSUMER_BALANCE_FRED_REUSED.map((r) => ({
        code: r.code,
    ...
    console.log("[verify-consumer-balance] Instrument metadata（仅本维度 seed 的 10 条）");
    let metaOk = 0;
    for (const row of CONSUMER_BALANCE_FRED_SERIES) {
      const inst = await prisma.instrument.findUnique({ where: { code: row.code } });
      if (!inst) continue;
    ...
    console.log(`  ✓ ${metaOk}/${CONSUMER_BALANCE_FRED_SERIES.length} 条 metadata 完整`);

    console.log("[verify-consumer-balance] 近期观测");
    let obsOk = 0;
    const allRows = [
      ...CONSUMER_BALANCE_FRED_SERIES.map((r) => ({
        code: r.code,
        fredId: r.fredId,
    ...
    console.log("[verify-consumer-balance] 历史深度（Spec §3 核实列）");
    let depthOk = 0;
    for (const row of CONSUMER_BALANCE_FRED_SERIES) {
      const inst = await prisma.instrument.findUnique({ where: { code: row.code } });
      if (!inst) continue;
    ...
    console.log(`  ✓ ${depthOk}/${CONSUMER_BALANCE_FRED_SERIES.length} 条历史深度符合预期`);
  } catch (e) {
  ...
  if (errors > 0) {
    console.error(`[verify-consumer-balance] 失败：${errors} 项`);
    process.exit(1);
  }
  console.log("[verify-consumer-balance] 通过");
}
