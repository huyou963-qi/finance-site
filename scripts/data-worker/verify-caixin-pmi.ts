/**
 * 中国制造业 PMI（民间口径，TE 抓取）——自检
 *
 * npm run data:verify-caixin-pmi
 * npm run data:verify-caixin-pmi -- --db
 */
...
async function main() {
  ...
  console.log(`[verify-caixin-pmi] 目标仪器 ${CAIXIN_PMI_INSTRUMENT_CODE}`);

  if (!useDb) {
    console.log("[verify-caixin-pmi] 通过（加 --db 检查数据库）");
    return;
  }

  const prisma = new PrismaClient();
  try {
    const code = CAIXIN_PMI_INSTRUMENT_CODE;
    const inst = await prisma.instrument.findUnique({ where: { code } });
    if (!inst) {
      console.error(`  ✗ 缺 Instrument ${code}（先 data:seed-caixin-pmi-te）`);
      errors++;
    } else {
      ...
      if (scrape?.provider !== CAIXIN_PMI_SCRAPE_PROVIDER) {
        console.error(`  ✗ scrape.provider=${scrape?.provider ?? "无"}（应 ${CAIXIN_PMI_SCRAPE_PROVIDER}）`);
        errors++;
      } else {
        console.log(`  ✓ scrape.provider=${CAIXIN_PMI_SCRAPE_PROVIDER} · url=${scrape.url}`);
      }
      ...
      if (count < 1) {
        console.error(`  ✗ 观测 0 条（先 data:sync-caixin-pmi-te）`);
        errors++;
      } else {
        console.log(
  ...
  if (errors > 0) {
    console.error(`[verify-caixin-pmi] 失败：${errors} 项`);
    process.exit(1);
  }
  console.log("[verify-caixin-pmi] 通过");
}
