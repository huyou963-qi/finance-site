/**
 * 标普500 市盈率（multpl.com）——自检
 *
 * npm run data:verify-multpl-sp500-pe
 * npm run data:verify-multpl-sp500-pe -- --db
 */
import { loadEnvConfig } from "@next/env";
loadEnvConfig(process.cwd());

import { PrismaClient } from "@prisma/client";
import { readFetchAcquisition } from "../../src/lib/data/scheduler/fetchAcquisition";
import {
  MULTPL_SP500_PE_INSTRUMENT,
  MULTPL_VALUATION_PACKAGE_ID,
} from "../../src/lib/data/scheduler/multplSp500Pe/catalog";

async function main() {
  let errors = 0;
  const useDb = process.argv.includes("--db");
  const code = MULTPL_SP500_PE_INSTRUMENT.code;
  console.log(`[verify-multpl-sp500-pe] 目标仪器 ${code}`);

  if (!useDb) {
    console.log("[verify-multpl-sp500-pe] 通过（加 --db 检查数据库）");
    return;
  }

  const prisma = new PrismaClient();
  try {
    const inst = await prisma.instrument.findUnique({ where: { code } });
    if (!inst) {
      console.error(`  ✗ 缺 Instrument ${code}（先 data:seed-multpl-sp500-pe）`);
      process.exit(1);
    }
    if (inst.freqLabel !== MULTPL_SP500_PE_INSTRUMENT.freqLabel || inst.unit !== MULTPL_SP500_PE_INSTRUMENT.unit) {
      console.error(`  ✗ freqLabel/unit=${inst.freqLabel}/${inst.unit}（应 月/倍）`);
      errors++;
    }

    const acq = readFetchAcquisition(inst.metadata);
    if (acq?.status !== "known") {
      console.error(`  ✗ fetchAcquisition.status=${acq?.status ?? "无"}（应 known）`);
      errors++;
    } else {
      console.log(`  ✓ 获取方式 known（${acq.methodLabel}）`);
    }
    const md = (inst.metadata ?? {}) as Record<string, unknown>;
    const scrape = md.scrape as Record<string, unknown> | undefined;
    if (scrape?.provider !== "multpl_sp500_pe") {
      console.error(`  ✗ scrape.provider=${scrape?.provider ?? "无"}（应 multpl_sp500_pe）`);
      errors++;
    } else {
      console.log(`  ✓ scrape.provider=multpl_sp500_pe · url=${scrape.url}`);
    }

    const sub = await prisma.dataSubscription.findUnique({ where: { instrumentId: inst.id } });
    if (!sub?.enabled) {
      console.error("  ✗ 订阅未启用");
      errors++;
    } else if ((sub.releaseRule as { type?: string })?.type !== "probe_interval") {
      console.error(`  ✗ releaseRule 应 probe_interval，实际 ${(sub.releaseRule as { type?: string })?.type}`);
      errors++;
    } else if (sub.releasePackageId !== MULTPL_VALUATION_PACKAGE_ID) {
      console.error(`  ✗ 发布包=${sub.releasePackageId ?? "无"}（应 ${MULTPL_VALUATION_PACKAGE_ID}，先 data:seed-release-packages）`);
      errors++;
    } else {
      console.log(
        `  ✓ 订阅启用，probe_interval，发布包 ${sub.releasePackageId}，nextRunAt=${sub.nextRunAt?.toISOString().slice(0, 10)}`,
      );
    }

    const count = await prisma.macroObservation.count({ where: { instrumentId: inst.id } });
    const first = await prisma.macroObservation.findFirst({ where: { instrumentId: inst.id }, orderBy: { obsDate: "asc" } });
    const last = await prisma.macroObservation.findFirst({ where: { instrumentId: inst.id }, orderBy: { obsDate: "desc" } });
    if (count < 1500) {
      console.error(`  ✗ 观测仅 ${count} 条（预期 ≥1500，1871-01 起约 1860 月）`);
      errors++;
    } else {
      console.log(
        `  ✓ 观测 ${count} 条 · ${first?.obsDate.toISOString().slice(0, 10)}(${first?.value}) → ${last?.obsDate.toISOString().slice(0, 10)}(${last?.value})`,
      );
    }
    const staleDays = last ? (Date.now() - last.obsDate.getTime()) / 86_400_000 : Infinity;
    if (staleDays > 75) {
      console.error(`  ✗ 最新观测滞后 ${Math.round(staleDays)} 天（月频应 ≤75）`);
      errors++;
    }
    const bad = await prisma.macroObservation.count({
      where: { instrumentId: inst.id, OR: [{ value: { lt: 1 } }, { value: { gt: 200 } }] },
    });
    if (bad > 0) {
      console.error(`  ✗ ${bad} 条观测值超出 [1,200]`);
      errors++;
    } else {
      console.log("  ✓ 全部观测值在 [1,200]");
    }
  } catch (e) {
    console.error(`  ✗ DB: ${e instanceof Error ? e.message : e}`);
    errors++;
  } finally {
    await prisma.$disconnect();
  }

  if (errors > 0) {
    console.error(`[verify-multpl-sp500-pe] 失败：${errors} 项`);
    process.exit(1);
  }
  console.log("[verify-multpl-sp500-pe] 通过");
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});
