/**
 * Damodaran 隐含股权风险溢价抓取——自检
 *
 * npm run data:verify-damodaran-erp
 * npm run data:verify-damodaran-erp -- --db
 */
...
async function main() {
  ...
  const code = DAMODARAN_ERP_INSTRUMENT.code;
  console.log(`[verify-damodaran-erp] 目标仪器 ${code}`);

  if (!useDb) {
    console.log("[verify-damodaran-erp] 通过（加 --db 检查数据库）");
    return;
  }
  ...
  try {
    ...
    if (!inst) {
      console.error(`  ✗ 缺 Instrument ${code}（先 data:seed-damodaran-erp）`);
      process.exit(1);
    }
    ...
    if (scrape?.provider !== "damodaran_erp") {
      console.error(`  ✗ scrape.provider=${scrape?.provider ?? "无"}（应 damodaran_erp）`);
      errors++;
    } else {
      console.log(`  ✓ scrape.provider=damodaran_erp · url=${scrape.url}`);
    }
    ...
    if (count < 60) {
      console.error(`  ✗ 观测仅 ${count} 条（预期 ≥60，全历史约 65 年 1961-2025）`);
      errors++;
    } else {
      console.log(
    ...
    // 值域检查：ERP 百分比应在合理区间 [0,20]
    const bad = await prisma.macroObservation.count({
      where: { instrumentId: inst.id, OR: [{ value: { lt: 0 } }, { value: { gt: 20 } }] },
    });
    if (bad > 0) {
      console.error(`  ✗ ${bad} 条观测值超出 [0,20]%（单位应为百分比）`);
      errors++;
    } else {
      console.log("  ✓ 全部观测值在 [0,20]%");
    }
  } catch (e) {
  ...
  if (errors > 0) {
    console.error(`[verify-damodaran-erp] 失败：${errors} 项`);
    process.exit(1);
  }
  console.log("[verify-damodaran-erp] 通过");
}
