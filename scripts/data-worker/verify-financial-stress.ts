/** 圣路易斯联储金融压力指数（STLFSI4）接入自检。 */
import { loadEnvConfig } from "@next/env";
import { PrismaClient } from "@prisma/client";
import { FINANCIAL_STRESS_FRED_SERIES } from "../../src/lib/data/scheduler/financialStressFredSeedCatalog";
...
async function main() {
  ...
  console.log(
    `[verify-financial-stress] ${FINANCIAL_STRESS_FRED_SERIES.length} 条；周频；St. Louis Fed Financial Stress Index`,
  );
  if (!useDb) {
    console.log("[verify-financial-stress] 静态目录通过（加 --db 检查数据库）");
    return;
  }
  ...
  let errors = 0;
  try {
    for (const item of FINANCIAL_STRESS_FRED_SERIES) {
      ...
      if (
        ...
        !latest ||
        aggregate._max.obsDate! < daysAgo(21)
      ) {
        console.error(
  ...
  console.log("[verify-financial-stress] 通过");
}
