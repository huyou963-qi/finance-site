...
async function main() {
  ...
  console.log("[data:seed-mofcom-trade] 正在低频回填商务部转载的海关货物贸易公开历史…");
  const history = await fetchMofcomTradeHistory({ historical: true }); const releaseRule = defaultEconomicCalendarRule(DataGranularity.MONTHLY); const nextRunAt = computeNextRunAt(releaseRule); const probedAt = new Date().toISOString(); let observations = 0;
  ...
  console.log(`[data:seed-mofcom-trade] 完成：序列=${history.size}，历史新增或修订=${observations}`);
}
