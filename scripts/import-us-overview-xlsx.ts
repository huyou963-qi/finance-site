...
import {
  matchUsOverviewHeader,
  US_OVERVIEW_CATEGORY_CODE_BY_NAME,
  US_OVERVIEW_CATEGORY_SORT_BY_NAME,
  US_OVERVIEW_SERIES,
} from "../src/lib/data/usOverviewLayout";

loadEnvConfig(process.cwd());

const prisma = new PrismaClient();

const DEFAULT_XLSX_PATH = "C:/Users/Administrator/Desktop/模板/US_Overview.xlsx";
const SOURCE_TAG = "us-overview-xlsx";
const COUNTRY_CODE = "US";
const COUNTRY_NAME_ZH = "美国";
...
function parseWorkbookSeries(sheetName: string, rows: unknown[][]): SeriesSeed[] {
  ...
  for (const def of US_OVERVIEW_SERIES) {
    ...
    if (!matchUsOverviewHeader(headerName, def)) {
      console.warn(
...
async function main() {
  ...
  const usCategoryId = await upsertCategory("macro_country_us", "美国", rootCategoryId, 10);

  const themeCategoryIds = new Map<string, string>();
  for (const name of new Set(US_OVERVIEW_SERIES.map((row) => row.catalogCategory))) {
    const slug = US_OVERVIEW_CATEGORY_CODE_BY_NAME[name];
    if (!slug) throw new Error(`missing category code mapping: ${name}`);
    const id = await upsertCategory(
      `macro_country_us_${slug}`,
      name,
      usCategoryId,
      US_OVERVIEW_CATEGORY_SORT_BY_NAME[name] ?? 500,
    );
  ...
  if (seriesList.length !== US_OVERVIEW_SERIES.length) {
    console.warn(
      `[warn] expected ${US_OVERVIEW_SERIES.length} series, parsed ${seriesList.length}`,
    );
  }
  ...
  for (const series of seriesList) {
    ...
    const instrument = await prisma.instrument.upsert({
      ...
      create: {
        code: series.code,
        kind: InstrumentKind.MACRO_SERIES,
        name: `US Overview:${series.displayName}`,
        shortName: series.displayName,
        description: "US Overview 导入序列",
