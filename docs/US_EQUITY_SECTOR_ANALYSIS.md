...
3. **用同一价格口径核验。** 前端只请求一次 `GET /api/equity/sector-returns?from=1998-12-16&to=2099-12-31&nav=1&assets=1` 完整净值序列，再在浏览器内按 30 个阶段的首尾可得交易日计算总回报和相对 SPY 超额，避免重复拉取 30 次全历史。
4. **先看大类资产，再看行业内部。** 阶段边界是宏观事件，一阶影响先落在利率、信用、实物与汇率上；阶段卡在 SPY 之上固定列出九条大类资产（利率：长期美债 / 短债 / TIPS；信用：投资级 / 高收益；实物：黄金 / 原油 / 商品综合；货币：美元指数），先回答“这段该不该拿股票”，再回答“股票内部拿谁”。资产集合的实证选择依据、与阶段的关系检验与已知限制见 [`docs/research/US_SECTOR_STAGE_ASSET_CLASSES.md`](./research/US_SECTOR_STAGE_ASSET_CLASSES.md)；定义在 `src/lib/equity/macroAssetClasses.ts`。
5. **最后解释偏差。** 理论受益不等于当期第一：估值起点、行业权重、商品价格、政策与事件风险都可能改变排序。缺少历史的序列不补造数据（XLC、XLRE 的样本期更短；黄金、原油、TIPS 的代理自 2000 年才起步）。

页面保持“上方全历史走势图 + 下方横向阶段卡”的草图结构。点击卡片只缩放主图窗口；每张卡固定列出九条大类资产、SPY 与全部 11 个行业，并同时显示绝对收益和相对 SPY 超额。

**阶段窗口是严格的**：首尾样本必须贴住阶段边界（容差 15 天且不超过窗口长度的 20%），否则该单元格记为 `—`。上市晚于阶段起点的序列（XLRE 2015-10、XLC 2018-06、GC=F 2000-08）此前会给出一段局部行情并被读成整段阶段收益，现已改为不可比；右边界统一钉在 SPY 最后一根日线上，服务端算大类资产与浏览器端算行业因此共用同一窗口。手工收益矩阵仍可加入任意自定义区间，供继续做事件前后切片。
