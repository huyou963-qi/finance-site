...
async function indicatorList(cid: string): Promise<Indicator[]> {
  const r = await fetchChinaOfficial(`${NBS_DATA_API_BASE}/new/queryIndicatorsByCid?cid=${cid}`, { headers, signal: AbortSignal.timeout(30_000) });
...
async function request(cid: string, indicatorIds: string[], startYear: number) {
  const r = await fetchChinaOfficial(`${NBS_DATA_API_BASE}/stream/esData`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ cid, indicatorIds, das: [{ text: "全国", value: "000000000000" }], dts: [`${startYear}01MM-${new Date().getUTCFullYear() + 1}12MM`], showType: "1", rootId: NBS_MONTHLY_ROOT_ID }), signal: AbortSignal.timeout(90_000) });
...
export async function fetchLatestIndustrialMom(): Promise<{ point: ObservationPoint; articleUrl: string }> {
  const index = await fetchChinaOfficial(NBS_INDUSTRIAL_INDEX_URL, { headers, signal: AbortSignal.timeout(30_000) }); if (!index.ok) throw new Error(`国家统计局工业发布目录 HTTP ${index.status}`);
  const links = [...(await index.text()).matchAll(/href="([^"]*\/\d{6}\/t\d+_\d+\.html)"/g)].map((x) => new URL(x[1]!, NBS_INDUSTRIAL_INDEX_URL).toString());
  for (const articleUrl of [...new Set(links)].slice(0, 40)) { const r = await fetchChinaOfficial(articleUrl, { headers, signal: AbortSignal.timeout(30_000) }); if (!r.ok) continue; const text = stripHtml(await r.text()); const m = /从环比看[，,](\d{1,2})月份[，,]规模以上工业增加值比上月(增长|下降)([\d.]+)%/.exec(text); const date = /(\d{4})年(\d{1,2})月份规模以上工业增加值/.exec(text); if (m && date) return { point: { obsDate: new Date(Date.UTC(Number(date[1]), Number(date[2]) - 1, 1)), value: (m[2] === "下降" ? -1 : 1) * Number(m[3]) }, articleUrl }; }
