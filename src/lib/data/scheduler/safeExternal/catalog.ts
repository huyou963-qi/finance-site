...
export const SAFE_DATASETS: readonly {
  key: SafeDataset; label: string; category: string; pages: readonly string[];
  /** Only workbook links whose title/text matches are read (landing pages also carry unrelated files). */
  attachmentTitle?: RegExp;
  /** The page publishes an English workbook that is translated back to the original Chinese table identity. */
  translate?: "settlement-en";
}[] = [
  ...
  // 中文时间序列页 /safe/2023/0215/22329.html 已于 2026-09 下线（404），改读英文站同一张表。
  { key: "settlement", label: "银行结售汇", category: "外汇收支与跨境资金", pages: ["https://www.safe.gov.cn/en/2023/0215/2048.html"], attachmentTitle: /^Time-series Data of Foreign Exchange Settlement and Sales by Banks/i, translate: "settlement-en" },
