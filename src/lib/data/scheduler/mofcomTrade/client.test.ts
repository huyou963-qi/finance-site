...
test("parses official MOFCOM customs-trade JSON rows", async () => {
  ...
  assert.equal(country?.points[0]?.obsDate.toISOString().slice(0, 10), "2026-05-01");
  // 分商品维度已迁到 gacc-commodity（海关量值表），这里不应再产出任何商品构成序列
  assert.equal([...values.values()].filter((item) => item.label.includes("商品构成")).length, 0);
});
