...
test("parses the nationwide monthly level series without the workbook YoY formulas", () => {
  const parsed = parseJpJntoVisitorArrivalsWorkbook(
    readFileSync(fixture("visitor-arrivals-2026-08-19.xlsx")),
    new Date("2026-09-13T00:00:00Z"),
  );
  assert.equal(Object.keys(parsed.series).length, 1);
...
test("fails closed when the required national-total row disappears", () => {
  ...
  const rowIndex = rows.findIndex((row) => row.some((cell) => String(cell ?? "").trim() === "総数"));
  const columnIndex = rows[rowIndex].findIndex((cell) => String(cell ?? "").trim() === "総数");
