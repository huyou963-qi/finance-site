...
export async function fetchNbsRetailIncremental(metadata: unknown, code: string, obsStart: string): Promise<FetchIncrementalResult> {
  ...
  const response = await fetchChinaOfficial(API, { method: "POST", headers: { "Content-Type": "application/json", Referer: "https://data.stats.gov.cn/dg/website/page.html", "User-Agent": process.env.NBS_USER_AGENT?.trim() || "finance-site-data-scheduler/1.0" }, body: JSON.stringify({ cid, indicatorIds: [indicatorId], das: [{ text: "全国", value: "000000000000" }], dts: ["200001MM-203012MM"], showType: "1", rootId: ROOT }), signal: AbortSignal.timeout(90_000) });
  ...
  const points = (json.data ?? []).flatMap(row => { const m = /^(\d{4})(\d{2})MM$/.exec(String(row.code ?? "")); const raw = row.values?.find(v => v._id === indicatorId)?.value; const value = Number(raw); return m && raw != null && Number.isFinite(value) ? [{ obsDate: new Date(Date.UTC(+m[1], +m[2] - 1, 1)), value }] : []; });
