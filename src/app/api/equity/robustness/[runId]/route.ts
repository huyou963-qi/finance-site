...
import { deleteRun, getRunDetail } from "@/lib/quant/robustnessRuns";

/**
 * 稳健性 run 详情（轮询）。?status=1 只回状态摘要（省去大 result）；否则 done 时附完整结果。
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ runId: string }> }) {
  try {
    ...
    if (!detail) return NextResponse.json({ error: "稳健性分析不存在" }, { status: 404 });
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ runId: string }> }) {
  try {
    ...
    if (!ok) return NextResponse.json({ error: "稳健性分析不存在" }, { status: 404 });
