...
export function AccountProfileClient() {
  ...
  if (!profile) {
    return <p className="text-sm text-slate-400">{hint}</p>;
  }

  return (
    <div className="space-y-4 rounded-lg border border-slate-800 bg-slate-950/60 p-4">
      <div>
        <h1 className="text-xl font-semibold text-slate-50">个人账户</h1>
      ...
      <div className="grid gap-3">
        <label className="text-sm text-slate-300">
          ...
          <input
            ...
            readOnly
            className="mt-1 w-full rounded-md border border-slate-800 bg-slate-900/50 px-2 py-1.5 text-slate-400"
          />
        </label>
        <label className="text-sm text-slate-300">
          会员类型
        ...
        <div className="rounded-md border border-slate-800 bg-slate-900/40 px-3 py-2">
          <p className="text-xs font-medium text-slate-400">修改密码（可选）</p>
