...
日本 e-Stat API 需服务端 `ESTAT_APP_ID`：`data:seed-jp-estat-cpi` / `data:sync-jp-estat-cpi` / `data:verify-jp-estat-cpi -- --db`；`data:seed-jp-estat-labor` / `data:sync-jp-estat-labor` / `data:verify-jp-estat-labor -- --db`；`data:seed-jp-estat-household` / `data:sync-jp-estat-household` / `data:verify-jp-estat-household -- --db`。CPI 使用 2025 基期官方接续指数；劳动力本批从 2018-01 开始，不能与更早人口基准数据硬拼；家计调查仅保存官方名义、未季调、每户月均金额，日文“实收入”不是实际收入指数。

日本厚生劳动省每月勤劳统计：`data:seed-jp-mhlw-monthly-labour` / `data:sync-jp-mhlw-monthly-labour` / `data:verify-jp-mhlw-monthly-labour -- --db`；走 e-Stat 官方长期时序 Excel，接入调查产业计、5人以上、就业形态计的2020基期工资/工时/常用雇用指数。旧 e-Stat 数据库表已停更，不得用于持续更新；长期表不与最新月速報硬拼。

日本内阁府机械订单：`data:seed-jp-esri-machinery-orders` / `data:sync-jp-esri-machinery-orders` / `data:verify-jp-esri-machinery-orders -- --db`；日本财务省外汇储备：`data:seed-jp-mof-reserves` / `data:sync-jp-mof-reserves` / `data:verify-jp-mof-reserves -- --db`；JNTO访日外客人数：`data:seed-jp-jnto-visitor-arrivals` / `data:sync-jp-jnto-visitor-arrivals` / `data:verify-jp-jnto-visitor-arrivals -- --db`。三域均按24小时探测官方滚动文件并完整回读历史以捕获修订；机械订单从2005-04现行口径开始，外汇储备不拼接2000-03以前旧标准，访日人数保留疫情期真实零值和低值。
...
「指标预测 · 美国 CPI」：顶栏「指标预测」→ `/forecast/us-cpi`（功能页 `forecast-us-cpi`，`/api/forecast/us-cpi`）；18 个 CPI 分项各用高频代理滚动回归、按 BLS 相对权重加总，页面同时展示 2018 年以来逐月样本外回测。代码 `src/lib/forecast/usCpi/`（纯函数模型有单测），只读既有宏观库序列，**模型输出不入库**（进程内缓存 30 分钟）；目标月要等全部 CPI 输入到齐才推进；高频数据按当月已观测天数取数，回测与 80% 区间按同一截止日校准（月末到公布前用全月）。**每年 1 月**在 `relativeImportance.ts` 追加 BLS 新一年相对权重。详见 [docs/research/US_CPI_NOWCAST.md](./docs/research/US_CPI_NOWCAST.md)。
「指标预测 · 美国非农」：`/forecast/us-nfp`（功能页 `forecast-us-nfp`，`/api/forecast/us-nfp`）；预测非农新增就业**首发值**：近 12 个月非农实时均值 + 参考周初请相对过去一年水平（验证期 2006–2015 从 48 组候选选定），按「今天在目标月第几天」同口径真实时点回测，初请每周四入库后自动更新。代码 `src/lib/forecast/usNfp/`；非农/ADP 历次版本读统一版本账本，`data:seed-forecast-vintages` 自 2002 年回填 ALFRED（幂等、随 `data:apply`），`data:verify-forecast-vintages -- --db` 自检。第三方对照：ADP（`fred:ADPMNUSNERSA` 并入 `seed-labor`，发布包 `us.adp.ner`）与费城联储 SPF（服务端读取不入库）。一致预期/Kalshi/Revelio/Indeed 均因条款不可用，详见 [docs/research/US_NFP_NOWCAST.md](./docs/research/US_NFP_NOWCAST.md)。
