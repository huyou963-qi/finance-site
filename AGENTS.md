...
⛔ **金额类汇总目前不可用，只有笔数与逐笔明细可信**（2026-09 实测结论，勿在此基础上做结论）：排除 anomaly 后仍有约 1,216 笔（占 0.022%）单笔名义额 >100 亿美元的行，它们贡献了全库 **99.92%** 的金额——即任何 sum 实际上都是在读这批错行。主导错误模式是**价格字段的量级错位**，且大盘股同样会犯：LLY 2023-08-28 报 `554101`（真实股价约 $554，×1000）、TGL 报 `933558`（真实约 $2）、KYN 报 `1000000`（真实约 $8）。已验证三种办法都不足以拦截：全局价格阈值（真实大宗交易亦可达十亿量级）、同发行人同季价格中位数（空壳股大部分行本身即错，中位数被污染）、限定 equity_security 宇宙（礼来这类大盘股照样填错）。**正确解法是拿 `mds.equity_daily_bar` 的当日实际股价交叉校验、对偏离达量级的行打标**，受限于日线覆盖范围；此项尚未实施。在实施之前，Tier B 只用于**笔数统计、逐笔明细与个人/公司维度的跟踪**，不要用它算美元金额。注意 1934–2004 之间的陈年交易**不是**错误（初次 Form 3 或更正时披露几十年前持仓属正常），故未标记，但做时间序列时应自行限定区间。
